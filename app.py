...
def render_sidebar():
    with st.sidebar:
        ...
        st.divider()
        num_weeks = st.radio("Number of Weeks", [4, 5], index=0, horizontal=True)
...
def main():
    ...
    root_path, folder_id, api_key, drive_service, save_destination, global_theme, uploaded_template, num_weeks = render_sidebar()
    tab1, tab2, tab3 = st.tabs(["📥 Data Import", "✅ Pre-Flight Check", "🚀 Generation"])

    with tab1:
        uploaded_data, dataframe, mapping = render_data_import()
    with tab2:
        render_photo_check(root_path, folder_id, api_key, drive_service, uploaded_data, dataframe, mapping.get("name"), num_weeks)
    with tab3:
        render_generation(root_path, folder_id, api_key, drive_service, save_destination, uploaded_template, uploaded_data, dataframe, mapping, global_theme, num_weeks)
