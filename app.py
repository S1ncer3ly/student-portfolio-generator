...
# ================= PAGE CONFIG =================
st.set_page_config(page_title="Student Portfolio Generator", page_icon="🎓", layout="wide")
...
# --- TAB 2: PRE-FLIGHT CHECK ---
with tab2:
    st.subheader("Step 2: Verify Photos")
    if st.button("🔍 Scan Folders for Photos"):
        if not root_path or uploaded_data is None:
            st.warning("Please provide the Root Folder Path and upload the Student List first!")
        else:
            if uploaded_data.name.endswith('.csv'):
                df = pd.read_csv(uploaded_data)
            else:
...
with tab3:
    ...
    if st.button("🔥 GENERATE ALL PRESENTATIONS", type="primary", use_container_width=True):
        if not uploaded_template or not uploaded_data or not root_path:
            st.error("Missing requirements! Please check the sidebar and Tab 1.")
        else:
            ...
            for index, row in df.iterrows():
                ...
                try:
                    ...
                    for w in range(1, 5):
                        ...
                        if placeholder:
                            ...
                            if os.path.exists(photo_path):
                                ...
                                placeholder.insert_picture(temp_jpg)
                                os.remove(temp_jpg)
                    ...
                    st.error(f"Error generating PPT for {name}: {e}")

                progress_bar.progress((index + 1) / len(df))

            status_text.success(f"🎉 Done! {len(df)} presentations created in: {output_folder}")
